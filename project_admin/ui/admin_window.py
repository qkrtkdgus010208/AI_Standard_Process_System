...
class AdminWindow(QMainWindow):
    ...
    def __init__(self, database_manager: DatabaseManager,
        ...
        self.product_service = product_service or ProductService(database_manager)
    ...
    def edit_selected_product(self, _index=None) -> None:
        ...
        if row < 0:
            ...
            return
        product_id = self.product_table.item(row, 0).text()
        product = self.product_service.get_for_edit(product_id)
        if product is None:
            QMessageBox.warning(self, "조회 실패", "제품을 찾을 수 없습니다.")
            return
        ...
        data = dialog.get_product_data()
        try:
            self.product_service.update(data)
            self.search_products()
        except Exception as error:
