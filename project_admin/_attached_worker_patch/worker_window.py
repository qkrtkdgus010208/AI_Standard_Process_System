...
import config
from auth_manager import ProductFetchThread, ProductInfo, WorkerSession
...
class WorkerWindow(QMainWindow):
    ...
    def _create_ui(self) -> None:
        ...
        self.mode_label = QLabel("TEST MODE" if config.TEST_MODE else "JETSON MODE")
        self.mode_label.setStyleSheet(
            "color:#315E91; background:#EEF4FA; border:1px solid #C9D9E9; "
            "border-radius:14px; padding:6px 11px; font-size:11px; font-weight:700;"
        )
        ...
        refresh_button = QPushButton("제품 새로고침")
        refresh_button.clicked.connect(self.fetch_products)
        apply_button = QPushButton("작업 정보 적용")
        apply_button.setObjectName("primaryButton")
        apply_button.clicked.connect(lambda: self.apply_work_setup(silent=False))
        btn_layout.addWidget(refresh_button)
        btn_layout.addWidget(apply_button)

        setup_grid.addLayout(btn_layout, 0, 5, alignment=Qt.AlignBottom)
    ...
    def update_work_state(self, state: dict) -> None:
        ...
        self.start_button.setEnabled(state_name in ("idle", "complete"))
    ...
    def _show_admin_message_popup(self, message_text: str) -> None:
        ...
        # Jetson 창 관리자에서도 숨지 않도록 실제 모달 Dialog로 표시합니다.
        popup.setModal(True)
